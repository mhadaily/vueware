...
export class MyoWebBluetooth {
    ...
    return service
      ...
      .then(char => {
        ...
        char.addEventListener(
          ...
          _this.handleBatteryLevelChanged
        );
    ...
    return server
      ...
      .then(_ => {
        ...
        let classifierEventChar = requestedCharacteristics.filter(char => {
          return (
            char.uuid == characteristics.classifierEventCharacteristic.uuid
          );
        });
        ...
        if (classifierService.length > 0) {
          ...
          _this.getClassifierData(
            classifierService[0],
            classifierEventChar[0],
            server
          );
        }
      })
  ...
  eventArmSynced(arm, x_direction) {
    ...
    const myoDirection =
      x_direction == 1 ? "wrist" : x_direction == 2 ? "elbow" : "unknown";
